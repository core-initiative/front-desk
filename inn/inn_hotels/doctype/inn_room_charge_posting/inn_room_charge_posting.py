...
def post_room_charges(tobe_posted_list):
	...
	for item in list_json:
		...
		room_charge_folio_trx.remark = 'Room Charge: Room Rate (Nett): ' + item['room_id'] + " - " + datetime.datetime.today().strftime("%d-%m-%Y")
		...
		breakfast_charge_folio_trx.remark = 'Room Charge: Breakfast (Nett): ' + item['room_id'] + " - " + datetime.datetime.today().strftime("%d-%m-%Y")
		...
		room_tb_id, room_tb_amount, _ = calculate_inn_tax_and_charges(reservation.nett_actual_room_rate, reservation.actual_room_rate_tax)
		for index, room_tax_item_name in enumerate(room_tb_id):
			...
			room_tax_doc.remark = 'Room Charge Tax Room Rate ' + room_tax_item_name + ' : ' + item['room_id'] + " - " + datetime.datetime.today().strftime("%d-%m-%Y")
			room_tax_doc.parent = item['folio_id']
			room_tax_doc.parenttype = 'Inn Folio'
			room_tax_doc.parentfield = 'folio_transaction'
			room_tax_doc.insert()

		breakfast_tb_id, breakfast_tb_amount, _ = calculate_inn_tax_and_charges(reservation.nett_actual_breakfast_rate, reservation.actual_breakfast_rate_tax)
